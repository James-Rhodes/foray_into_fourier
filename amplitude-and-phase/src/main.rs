...
async fn main() {
    ...
    loop {
        ...
        graph.draw_axes();
        graph.plot_line_xy(&time, &sine_wave, 2., ORANGE);

        let mouse_pos = animation.get_world_mouse();

        let amp_pos = vec2(-250., 300.);
        draw_text_centered("Amplitude", amp_pos.x, amp_pos.y + 25., 25, WHITE);
        mqanim::ui::Slider::new(amp_pos, slider_size, -1.0..1.)
            .mouse_pos(mouse_pos)
            .draw(&mut amp);

        let phase_pos = vec2(250., 300.);
        draw_text_centered("Phase", phase_pos.x, phase_pos.y + 25., 25, WHITE);
